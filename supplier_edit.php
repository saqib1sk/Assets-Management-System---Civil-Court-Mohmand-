...
require_once('my_connection.php');
    ...
    if(isset($_GET['id'])){
         $id = $_GET['id'];
       //echo $id;
    $select_qry = mysqli_query($my_connection, "SELECT * FROM `suppliers` where id = $id") or die(mysqli_error($my_connection));
...
?>
 ...
 if(@$_FILES["file_path"]["name"][0] != null){
  ...
  $target_dir = "employee_images/".$file_name;
   ...
   $update_query = mysqli_query($my_connection,"UPDATE `suppliers` SET `name`='$name',`mobile`='$mobile',`cnic`='$cnic',`email`='$email',`image_path`='$target_dir' WHERE id = '$id'") or die (mysqli_error($my_connection));
echo "<script>
window.location = 'supplier_add.php?msg=updated';
</script>"; 
//  header('location:user_registration.php?msg=updated');
 }
 else{
//ubdate record with out file attached...
    $update_query = mysqli_query($my_connection,"UPDATE `suppliers` SET `name`='$name',`mobile`='$mobile',`cnic`='$cnic',`email`='$email' WHERE id = '$id'") or die (mysqli_error($my_connection));
...
<script src="assets/js/bootstrap-fileupload.js"></script>
            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <h4 class="page-title">Edit Vendor Details</h4>
                        <ol class="breadcrumb">
                        </ol>
                    </div>
                </div>
   ...
   <form class="" role="form" method="post" enctype="multipart/form-data">
                                      ...
                                      //  echo "select path from product_images where product_id = '$id'";
                                        $id = $_GET['id'];
                                        $select_qry2 = mysqli_query($my_connection, "select id,image_path from suppliers where id = '$id'") 
